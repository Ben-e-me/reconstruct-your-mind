...
// hue=244, hoverIntensity=0.6, forceHoverState=true. Animation runs at 50% speed.
...
const TIME_SCALE = 0.5 // 50% slower than the reference

export function Orb({ active = true, delay = 0 }: { active?: boolean; delay?: number }) {
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      hue: { value: HUE },
      hover: { value: 1 }, // forceHoverState
      rot: { value: 0 },
      hoverIntensity: { value: HOVER_INTENSITY },
      backgroundColor: { value: new THREE.Vector3(0, 0, 0) },
    }

    const material = new THREE.ShaderMaterial({ vertexShader, fragmentShader, uniforms, transparent: true })
    // the shader outputs premultiplied-alpha colour (rgb*a, a) — composite it that way
    ...
    const resize = () => {
      ...
      uniforms.iResolution.value.set(w * dpr, h * dpr, w / h)
    }
    resize()
    ...
    const animate = (t: number) => {
      ...
      const time = ((t - t0) / 1000) * TIME_SCALE
      uniforms.iTime.value = time
      uniforms.rot.value = time * 0.3 // gentle rotation
